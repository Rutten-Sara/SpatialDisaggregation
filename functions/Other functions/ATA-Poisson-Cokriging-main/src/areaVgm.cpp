...
//#pragma omp parallel for private(i, j, areaIndexI, areaIndexJ, indexI, indexJ, x1, y1, x2, y2, w1, w2)
	for (i = 0; i < uId.size() - 1; i++)
	{
		...
		//--------------------------------------------------- IMPORTANT ----------
		g_areaWeightByPtsX.push_back(1/sum(outerProd(w1, w1)) * outerProd(w1, w1));
		//--------------------------------------------------- IMPORTANT ----------
		...
		*/
		for (j = i+1; j < uId.size(); j++)
		{
			...
			//--------------------------------------------------- IMPORTANT ----------
			g_areaWeightByPtsX.push_back(1/sum(outerProd(w2, w2)) * outerProd(w2, w2));
			g_areaWeightByPtsX.push_back(1/sum(outerProd(w1, w2)) * outerProd(w1, w2));
			//--------------------------------------------------- IMPORTANT ----------
