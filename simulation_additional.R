...
time_knots2 <- (proc.time()-mtime_knots2)[3]
    ...
    result <- data.frame(
      ...
      SSDAM = round(c(mean(time_raster),mean(Bias_spat_raster),
                         sqrt(mean(RMSE_spat_raster, na.rm = T)), mean(CI_spat_raster),
                         mean(Bias_continuous_incidence),
                         sqrt(mean(RMSE_continuous_incidence, na.rm=T)),
                         mean(CI_continuous_incidence),
                         mean(Bias_area), 
                         sqrt(mean(RMSE_area, na.rm = T)),
                      mean(CI_area)),2),
      SSDEM = round(c(mean(time_exact),mean(Bias_spat_exact),
                      sqrt(mean(RMSE_spat_exact, na.rm = T)), mean(CI_spat_exact),
                      mean(Bias_continuous_incidence_exact),
                      sqrt(mean(RMSE_continuous_incidence_exact, na.rm=T)),
                      mean(CI_continuous_incidence_exact),
                      mean(Bias_area_exact), 
                      sqrt(mean(RMSE_area_exact, na.rm = T)),
                      mean(CI_area_exact)),2)
